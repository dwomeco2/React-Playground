    ...
    return (
        <div className="w-full h-1 rounded-lg relative mt-12 mb-16">
            <input 
                ...
                max={100} 
                type="range" 
                className="slider-bar w-full appearance-none" 
...
function Toggle({ className = '' }: { className?: string }) {
    ...
    let discount_background_color = "bg-[var(--discount-background-color)]";
    let discount_on_background_color = "text-[var(--discount-on-background-color)]";
        ...
        <div className={className}>
            <div className="flex justify-center relative">
                <div className="text-sm text-gray-600 mr-2">Monthly Billing</div>
                    <button 
                        className={`pricing_slider_toggle ${isMonthly ? 'active': ''} rounded-full`} 
...
function Card({ children }: { children?: JSX.Element | JSX.Element[] }) {
    return (
        <div className="slider-container w-[36rem] rounded-md bg-white p-8 overflow-hidden">
            {children}
        </div>
    )
}
