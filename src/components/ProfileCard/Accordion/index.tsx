	...
	return (
		<div className='flex mb-4'>
			...
			<div className={`flex flex-col text-start w-full ${props.color2}`}>
				<button
					type='button'
					className='flex' onClick={onToggle}
				>
					<div className='flex-grow text-start pl-2 my-3'>{props.title}</div>
					<div
						className={
							'w-12 self-center transition-transform '
							+ (props.isToggled ? 'rotate-45' : '')
						}
					>
						+
					</div>
				...
				<div
					className={
						'pl-2 pr-12 transition-all overflow-hidden '
						+ (props.isToggled ? 'p-2 max-h-screen' : 'max-h-0')
					}
				>
					{props.content}
				</div>
	...
	const toggleContent = (index: number) => {
		setAccordionItems(prev => {
			...
			newAccordionItems[index] = {
				...newAccordionItems[index],
				isToggled: !newAccordionItems[index].isToggled,
			};
		...
		<div className='flex'>
			<div className='mx-auto sm:w-[560px]'>
				<div className='my-4 text-center text-[var(--colar-sand-2)]'>
					<h3 className='my-1 font-bold text-3xl'>
