...
const todoItemZod = z.object({ id: z.string(), text: z.string(), checked: z.boolean().default(false) })
    ...
    <div ref={ref}>
      <div className="flex items-center hover:bg-gray-800 rounded-md">
        <div className="flex grow ">
          ...
          <button type="button" onClick={() => deleteTodoItem(todo.id)} className="text-gray-200 text-lg py-2 px-1 hover:bg-gray-800 rounded-md">
            <FaTrash />
          </button>
        </div>
      </div>
...
const TodoContainer = ({ children, className }: { children: JSX.Element[] | JSX.Element; className: string }) => {
  const [, drop] = useDrop(() => ({
    accept: 'todo'
  }))
      ...
      <DndProvider backend={HTML5Backend}>
        <TodoContainer className="list-container max-h-[32rem]">
          {todos.map((item, index) => (
            <TodoItem key={item.id} index={index} handleTodoChecked={handleTodoChecked} handleDrop={handleDrop} deleteTodoItem={deleteTodoItem} {...item} />
          ))}
        </TodoContainer>
      </DndProvider>
    </div>
