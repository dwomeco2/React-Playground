...
export const useContentQuery = ({data, page}: UseContentQueryProps) => {
	...
	const [totalPages, itemIds, kidsQueries] = useInfiniteItemQueries(page, global.maxCommentsPerPage, data.kids);
...
export const useTopStoriesList = ({page}: UseTopStoriesListProps) => {
	...
	const [totalPages, itemIds, topStoriesQueries] = useInfiniteItemQueries(page, global.maxPageItems, topStoriesIdsQuery.data);

	return [totalPages, itemIds, topStoriesQueries] as UseInfiniteItemQueriesProps;
};

const useTopStoriesIdsQuery = () => useQuery({
	queryKey: ['topStories'],
...
const useItemQuery = (itemID: number, isSuspense = false) => useQuery({
...
export const useItemQueries = (itemIDs: number[] = []) => useQueries({
